...
public class Login_Controller {
	...
	public String register(@ModelAttribute PatientData patientdata, HttpSession session, String confirm_password) {
	
		int temp = paitentservice.saveData(patientdata, confirm_password);	
		if(temp==1)
		{
			session.setAttribute("message", "Registeration successful please login! ");
		}else if(temp==2){
			session.setAttribute("message", "Password and Confirm password not matching");
		}else{
