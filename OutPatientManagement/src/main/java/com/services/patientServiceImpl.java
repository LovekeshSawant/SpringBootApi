...
public class patientServiceImpl implements paitentService {
	...
	@Override
	public int saveData(PatientData patientedata, String confirm_password)
	{
		List<PatientData> p = detaildao.findBypemail(patientedata.getPemail());
		if(p.size()==0)
		{
			detaildao.save(patientedata);
			return 1;
		}else if(patientedata.getPassword().equals(confirm_password)==false) {
			return 2;
		}
		return 0;
	}
